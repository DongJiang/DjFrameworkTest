  ...
  const std::vector<std::vector<TDescriptor> > &training_features)
{
  ...
  std::cout<<"create m_nodes before reserve: "<<m_nodes.size()<<std::endl;
  std::cout<<"create expected_nodes before reserve:"<<expected_nodes<<std::endl;
  m_nodes.reserve(expected_nodes); // avoid allocations when creating the tree
  std::cout<<"create m_nodes after reserve: "<<m_nodes.size()<<std::endl;
  
  
  std::vector<pDescriptor> features;
  getFeatures(training_features, features);
  std::cout<<"create m_nodes after getFeatures:"<<m_nodes.size()<<std::endl;


  // create root  
  m_nodes.push_back(Node(0)); // root
  std::cout<<"create m_nodes after root push:"<<m_nodes.size()<<std::endl;
  
  // create the tree
  HKmeansStep(0, features, 1);
  std::cout<<"create m_nodes HKmeansStep:"<<m_nodes.size()<<std::endl;
  ...
  std::cout<<"m_nodes:"<<m_nodes.size()<<std::endl;
  std::cout<<"m_words:"<<m_words.size()<<std::endl;
...
void TemplatedVocabulary<TDescriptor,F>::createWords()
{
  ...
  if(!m_nodes.empty())
  {
      std::cout<<"m_words createWords reserving"<<(int)pow((double)m_k, (double)m_L)<<std::endl;
    m_words.reserve( (int)pow((double)m_k, (double)m_L) );

      std::cout<<"m_words createWords:"<<m_words.size()<<std::endl;
      std::cout<<"m_words createWords "<<m_k<<" "<<m_L<<std::endl;
